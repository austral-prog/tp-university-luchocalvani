...
import com.university.CLI.exceptions.InvalidInputException;
            ...
            try {
                if (entityClass == Student.class) {
                    ...
                    switch (evalTypeChoice) {
                        case 1 -> {
                            return entityClass.cast(new WrittenExam(studentName, subject, "WrittenExam", evalName));
                        }
                        case 2 -> {
                            return entityClass.cast(new OralExam(studentName, subject, "OralExam", evalName));
                        }
                        case 3 -> {
                            return entityClass.cast(new PracticalWork(studentName, subject, "PracticalWork", evalName));
                        }
                        case 4 -> {
                            return entityClass.cast(new FinalPracticalWork(studentName, subject, "FinalPracticalWork", evalName));
                        }
